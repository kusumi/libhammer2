...
pub fn get_volume_data_offset(index: usize) -> u64 {
    u64::try_from(index).unwrap() * fs::HAMMER2_ZONE_BYTES
}
...
pub(crate) fn read_volume_data(path: &str) -> std::io::Result<fs::Hammer2VolumeData> {
    ...
    for i in 0..fs::HAMMER2_NUM_VOLHDRS {
        let offset = get_volume_data_offset(i);
        if offset >= size {
            break;
        }
        util::seek_set(&mut fp, offset)?;
        let mut buf = vec![0; fs::HAMMER2_VOLUME_BYTES.try_into().unwrap()];
        fp.read_exact(&mut buf)?;
        let vd = util::align_to::<fs::Hammer2VolumeData>(&buf);
        // verify volume header magic
        if vd.magic != fs::HAMMER2_VOLUME_ID_HBO && vd.magic != fs::HAMMER2_VOLUME_ID_ABO {
            log::error!("{path} #{i}: bad magic");
            continue;
        }
        if vd.magic == fs::HAMMER2_VOLUME_ID_ABO {
            // XXX: Reversed-endianness filesystem
            log::error!("{path} #{i}: reverse-endian filesystem detected");
            continue;
        }
        // verify volume header CRC's
        let a = vd.icrc_sects[fs::HAMMER2_VOL_ICRC_SECT0];
        let b = vd.get_crc(fs::HAMMER2_VOLUME_ICRC0_OFF, fs::HAMMER2_VOLUME_ICRC0_SIZE);
        if a != b {
            log::error!("{path} #{i}: volume header crc mismatch sect0 {a:08x}/{b:08x}");
            continue;
        }
        let a = vd.icrc_sects[fs::HAMMER2_VOL_ICRC_SECT1];
        let b = vd.get_crc(fs::HAMMER2_VOLUME_ICRC1_OFF, fs::HAMMER2_VOLUME_ICRC1_SIZE);
        if a != b {
        ...
        let b = vd.get_crc(
            fs::HAMMER2_VOLUME_ICRCVH_OFF,
            fs::HAMMER2_VOLUME_ICRCVH_SIZE,
        );
        if a != b {
